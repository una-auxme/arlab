"""Generic manipulation action wrapper for robot motion commands.

Provides a reusable ROS2 action behavior that sends `ManipulationAction` goals
...
class GenericManipulation(RosActionBehaviour):
    ...
    This class wraps the `ManipulationAction` action server located at
    `MANIPULATION_ACTION_PATH` and is intended to be plugged into a behavior
    tree framework.
    ...
    Attributes:
        action_type: The ROS2 action type used (`ManipulationAction`).
    ...
    Notes:
        ...
        - `GenericManipulation` only configures the base class with the correct
          action type and server name.
    """

    def __init__(self, name: str, key: str, result_output_key: str):
        """Initialize the generic manipulation action behaviour.

        Args:
            name: Name of the behavior node (for debugging/visualization).
            key: Blackboard key used as input for the `ManipulationAction` goal.
