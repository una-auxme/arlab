...
def get_tree() -> Behaviour:
    return Sequence(
        name="move_to_in_cupboard",
        memory=True,
        children=[
            SetupManipulationMove(),
            GenericManipulation(
                "move_to_in_cupboard",
                "/manipulation/move_to_in_cupboard_goal",
                "/manipulation/move_to_in_cupboard_result",
            ),
...
class SetupManipulationMove(Behaviour):
    def __init__(self):
        ...
        self.blackboard.register_key(
            key="/manipulation/move_to_in_cupboard_goal", access=Access.WRITE
        )
...
class CheckManipulationMove(Behaviour):
    def __init__(self):
        ...
        self.blackboard = self.attach_blackboard_client(name=self.name)
        self.blackboard.register_key(
            key="/manipulation/move_to_in_cupboard_result", access=Access.READ
        )

    def update(self):
        result: ManipulationAction.Result = (
            self.blackboard.manipulation.move_to_in_cupboard_result
        )
